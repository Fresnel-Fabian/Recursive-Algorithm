...
class Test(unittest.TestCase):
    ...
    def test_integer_functions(self):
        ...
        variables = variables_init
        try: # Not in the students test
            wtok = TokenizeWrapper('fac(3.)')
            result = statement(wtok, variables)
            self.assertEqual(int, type(result), msg=f'fac is returning {type(result)} instead of int')
            wtok = TokenizeWrapper('fib(3.)')
            result = statement(wtok, variables)
            self.assertEqual(int, type(result), msg=f'fib is returning {type(result)} instead of int')
        except Exception as ee: # Not in the students test
            print('\nE ******* fib or fac not returning int ********')
            print(ee)
            print('Contunue\n')
        tests = {
            'fac(3)': 6,
